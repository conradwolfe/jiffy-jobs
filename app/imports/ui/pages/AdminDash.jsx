...
class AdminDash extends React.Component {
  ...
  render() {
    return (this.props.ready) ? this.renderPage() : <Loader active>Loading All Users</Loader>;
  }
          ...
          <div className="dashboard-background">
            <Container>
              <Card.Group centered itemsPerRow={4}>
                {this.props.companyinfo.map((companyinfo, index) => <CompanyCard
                    key={index} companyinfo={companyinfo}/>)}
              </Card.Group>
            </Container>
            <Container>
              <Card.Group centered itemsPerRow={4}>
                {this.props.studentinfo.map((studentinfo, index) => <StudentCard
                    key={index} studentinfo={studentinfo}/>)}
              </Card.Group>
            </Container>
          </div>
          <div className="dashboard-background">
...
export default withTracker(() => {
  ...
  const subscription1 = Meteor.subscribe('CompanyInfo');
  const subscription2 = Meteor.subscribe('StudentInfo');
  return {
